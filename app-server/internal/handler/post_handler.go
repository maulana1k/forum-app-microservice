...
func (h *PostHandler) GetUserPosts(c *fiber.Ctx) error {
	...
	posts, err := h.postService.GetPostsByUserID(uint(userID), page, limit)
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(dto.ErrorResponse{
			Error: err.Error(),
		})
...
func (h *PostHandler) LikePost(c *fiber.Ctx) error {
	...
	err = h.postService.LikePost(uint(postID), userID)
	if err != nil {
		switch err.Error() {
		case "post not found":
			return c.Status(fiber.StatusNotFound).JSON(dto.ErrorResponse{
...
func (h *PostHandler) UnlikePost(c *fiber.Ctx) error {
	...
	err = h.postService.UnlikePost(uint(postID), userID)
	if err != nil {
		switch err.Error() {
		case "post not liked":
			return c.Status(fiber.StatusBadRequest).JSON(dto.ErrorResponse{
...
func (h *PostHandler) BookmarkPost(c *fiber.Ctx) error {
	...
	err = h.postService.BookmarkPost(uint(postID), userID)
	if err != nil {
		switch err.Error() {
		case "post not found":
			return c.Status(fiber.StatusNotFound).JSON(dto.ErrorResponse{
...
//	@Failure		401	{object}	dto.ErrorResponse
//	@Failure		500	{object}	dto.ErrorResponse
//	@Router			/posts/{id}/unbookmark [delete]
func (h *PostHandler) UnbookmarkPost(c *fiber.Ctx) error {
	...
	err = h.postService.UnbookmarkPost(uint(postID), userID)
	if err != nil {
		switch err.Error() {
...
//	@Failure		401	{object}	dto.ErrorResponse
//	@Failure		500	{object}	dto.ErrorResponse
//	@Router			/posts/{id}/unrepost [delete]
